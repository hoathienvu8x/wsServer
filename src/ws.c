...
static void *close_timeout(void *p) {
  ...
  while (
    ...
    pthread_cond_timedwait(
      &conn->cnd_state_close, &conn->mtx_state, &ts
    ) != ETIMEDOUT
  );
 ...
 * for completeness.
 */
static int ws_sendframe_internal(
  ws_cli_conn_t *client, const char *msg,
...
int ws_close_client(ws_cli_conn_t *client) {
  ...
  if (
    ws_sendframe(
      client, (const char *)clse_code,
      sizeof(char) * 2, WS_FR_OP_CLSE
    ) < 0
  ) {
...
static int do_close(struct ws_frame_data *wfd, int close_code) {
  ...
  if (
    (cc < 1000 || cc > 1003) && (cc < 1007 || cc > 1011) &&
    (cc < 3000 || cc > 4999)
  ) {
    cc = WS_CLSE_PROTERR;
  ...
  custom_close:
    ...
    if (
      ws_sendframe(
        wfd->client, (const char *)wfd->msg_ctrl,
        sizeof(char) * 2, WS_FR_OP_CLSE
      ) < 0
    ) {
      DEBUG("An error has occurred while sending closing frame!\n");
 ...
 * for completeness.
 */
static int validate_utf8_txt(
  struct ws_frame_data *wfd, struct frame_state_data *fsd
) {
#ifdef VALIDATE_UTF8
  ...
  fsd->utf8_state = is_utf8_len_state(
    fsd->msg_data + (fsd->msg_idx_data - fsd->frame_length),
    fsd->frame_length, fsd->utf8_state
  );
...
static int next_complete_frame(struct ws_frame_data *wfd) {
  ...
  do
  {
     ...
     */
    if (
      (wfd->frame_type == -1 && fsd.opcode == WS_FR_OP_CONT) ||
      (
    ...
    if (
      get_client_state(wfd->client) == WS_STATE_CLOSING &&
...
static void *ws_accept(void *data) {
  ...
  while (1) {
    ...
    for (i = 0; i < MAX_CLIENTS; i++)
    {
      ...
      {
        memcpy(
          &client_socks[i].ws_srv, &ws_prm->ws_srv,
